...
let sign_of_cmi ~freshen { Persistent_env.Persistent_signature.cmi; _ } =
  Js_of_ocaml.Firebug.console##log "sign of cmi";
  let name = cmi.cmi_name in
  Js_of_ocaml.Firebug.console##log name;
...
let find_pers_mod name =
  Js_of_ocaml.Firebug.console##log name;
  Js_of_ocaml.Firebug.console##log !persistent_env;
  try 
    let v = Persistent_env.find !persistent_env read_sign_of_cmi name in
    Js_of_ocaml.Firebug.console##log v;
    v
  with Not_found -> 
    Js_of_ocaml.Firebug.console##log "NADAAAA";
    Js_of_ocaml.Firebug.console##log name;
    raise Not_found
...
let may_lookup_error report_errors loc env err =
  if report_errors then begin 
    Js_of_ocaml.Firebug.console##log (env.summary);
    lookup_error loc env err
  end
...
let lookup_ident_module (type a) (load : a load) ~errors ~use ~loc s env =
  Js_of_ocaml.Firebug.console##log "LIDENT1";
  let path, data =
  ...
  in
  Js_of_ocaml.Firebug.console##log "LIDENT2";
  match data with
  | Mod_local mda -> begin
      Js_of_ocaml.Firebug.console##log "LIDENT3";
      use_module ~use ~loc path mda;
      match load with
      | Load -> path, (mda : a)
      | Don't_load -> path, (() : a)
    end
  | Mod_unbound reason -> begin
      Js_of_ocaml.Firebug.console##log "LIDENT4";
      report_module_unbound ~errors ~loc env reason
  end
  | Mod_persistent -> begin
      Js_of_ocaml.Firebug.console##log "LIDENT5";
      match load with
      | Don't_load ->
          check_pers_mod ~loc s;
          path, (() : a)
      | Load -> begin
          Js_of_ocaml.Firebug.console##log "LIDENT LOAD";
          match find_pers_mod s with
          | mda ->
              Js_of_ocaml.Firebug.console##log "USE MOD";
              use_module ~use ~loc path mda;
              Js_of_ocaml.Firebug.console##log "PAST";
              path, (mda : a)
          | exception Not_found ->
              Js_of_ocaml.Firebug.console##log "NOT FGOUND";
...
let lookup_module_path ~errors ~use ~loc ~load lid env : Path.t =
  ...
  Js_of_ocaml.Firebug.console##log lid;
  match lid with
  | Lident s ->
      Js_of_ocaml.Firebug.console##log "LIDENT";
      if !Clflags.transparent_modules && not load then
        fst (lookup_ident_module Don't_load ~errors ~use ~loc s env)
      else
        fst (lookup_ident_module Load ~errors ~use ~loc s env)
  | Ldot(l, s) -> 
    Js_of_ocaml.Firebug.console##log "LDOT";
    fst (lookup_dot_module ~errors ~use ~loc l s env)
  | Lapply _ as lid ->
    Js_of_ocaml.Firebug.console##log "LAPP";
